    ...
    internal class UserData_TPS
    {
        ...
        public void userdata_TPS(IWebDriver driver, ISelenium selenium, datarow datarow)
        {
            try
            {
                ...
                IWebElement myDynamicElement8 =
                    driver.FindElement(
                        By.XPath("//body[@id='page-checkout-process']/div/div[2]/div/form/fieldset/div[2]/div/button"));

                decimal count =
                    selenium.GetXpathCount("//body[@id='page-checkout-process']/div/div[2]/div/form/section/div");
                if (count == 0)
                {
                ...
                for (int i = 1; i <= count; i++)
                {
                    if (
                        selenium.IsElementPresent(
                            "//body[@id='page-checkout-process']/div/div[2]/div/form/section/div[" + i + "]/label"))
                    {
                        string valuet =
                            driver.FindElement(
                                By.XPath("//body[@id='page-checkout-process']/div/div[2]/div/form/section/div[" + i +
                                         "]/label")).Text;
                            ...
                            try
                            {
                                ...
                                foreach (string value in varinats)
                                {
                                    if (!value.Contains("Please"))
                                    {
                                        ...
                                        {
                                            values = values + "\r\n" + value;
                                            new SelectElement(driver.FindElement(By.Id("FormData_" + j + "__Value")))
                                                .SelectByText(value);
                                            break;
                                        }
                                        else
                        ...
                        if (valuet.Contains("Email"))
                        {
                            try
                            {
                                ...
                                driver.FindElement(
                                    By.XPath("//body[@id='page-checkout-process']/div/div[2]/div/form/section/div[" + i +
                                             "]/input[2]")).SendKeys("test@test.com");
                            }
                            catch (Exception ex)
                            {
                                string e = ex.ToString();
